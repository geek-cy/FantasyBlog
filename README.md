...
本项目是在AlanLiang1998的GeekBlog基础上进行改进，优化了部分BUG，新增部分功能

1、权限管理模块：基于RBAC模型构建，并集成安全框架SpringSecurity，实现用户登录的认证和操作权限
2、日志模块：通过SpringBoot AOP获取用户操作并记录日志
3、缓存功能：使用Spring Data集成缓存中间件Redis，达到缓解数据库的压力目的
4、全文搜索：使用Spring Data集成全文搜索搜索引擎ElasticSearch，实现文章信息的快速搜索和关键字的高亮显示
5、定时任务：使用Spring集成Quartz,实现缓存持久化到数据库的功能
6、